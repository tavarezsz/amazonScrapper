...
app.get('/search',async(require,res) => {
  ...
  const { data: html } = await axios.get(url, {
    headers: { //use user-agent to seem more "human"
      ...
      'Accept-Language': 'en-US,en;q=0.9,pt-BR,pt;q=0.9'
    },
  });
    ...
    items.forEach((item) => {
      ...
      let rating = item.querySelector('span.a-icon-alt')?.textContent.trim()|| '0';
      ...
      if(price != null && priceFraction != null)
        ...
        price = 'see options' //some prices are not available because there are diferent models to choose from
